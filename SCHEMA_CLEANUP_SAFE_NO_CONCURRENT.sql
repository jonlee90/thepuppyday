-- SAFE Schema Cleanup Operations (Supabase SQL Editor Compatible)
-- Generated: 2025-12-27
-- Database: thepuppyday (Supabase PostgreSQL)
--
-- This version removes CONCURRENTLY keyword to work in Supabase SQL Editor.
-- These operations are SAFE to execute with minimal risk.
-- They improve performance and remove confirmed unused structures.
--
-- ⚠️ IMPORTANT DIFFERENCES FROM SCHEMA_CLEANUP_SAFE.sql:
-- - CREATE INDEX without CONCURRENTLY may cause brief table locks
-- - For small-medium databases (< 100k rows), locks are typically < 1 second
-- - For production databases with high traffic, consider running during low-traffic periods
-- - Alternative: Use MCP Supabase tools to run CONCURRENTLY indexes (see instructions below)
--
-- INSTRUCTIONS:
-- 1. Review each operation before running
-- 2. Run during low-traffic period (early morning recommended)
-- 3. Take database backup first (Supabase automatically backs up daily)
-- 4. Execute using Supabase SQL Editor
-- 5. Monitor table sizes and query performance after execution
--
-- TRANSACTION BLOCK LIMITATION:
-- Supabase SQL Editor wraps all SQL in a transaction block, which prevents
-- CREATE INDEX CONCURRENTLY from running. This version uses regular CREATE INDEX,
-- which may cause brief locks but works in SQL Editor.
--
-- FOR ZERO-DOWNTIME INDEX CREATION (Advanced):
-- Use MCP Supabase tools to run each index separately:
-- /mcp supabase apply_migration "add_idx_appointments_scheduled_at" "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_appointments_scheduled_at ON appointments(scheduled_at);"
...
CREATE INDEX IF NOT EXISTS idx_appointments_groomer_id
...
CREATE INDEX IF NOT EXISTS idx_notifications_log_status_created
...
CREATE INDEX IF NOT EXISTS idx_notifications_log_type_channel
...
CREATE INDEX IF NOT EXISTS idx_calendar_sync_log_appointment_id
...
CREATE INDEX IF NOT EXISTS idx_calendar_connections_admin_id
...
CREATE INDEX IF NOT EXISTS idx_calendar_connections_is_active
...
CREATE INDEX IF NOT EXISTS idx_waitlist_status_requested_date
...
CREATE INDEX IF NOT EXISTS idx_pets_owner_id_is_active
...
CREATE INDEX IF NOT EXISTS idx_report_cards_created_at
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE 'IMPORTANT: Indexes created without CONCURRENTLY';
  RAISE NOTICE '- Brief table locks occurred during index creation';
  RAISE NOTICE '- For databases < 100k rows, locks are typically < 1 second';
