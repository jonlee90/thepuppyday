/**
 * CSV Data Import Script for The Puppy Day
 *
 * Imports ~267 customers, ~330 pets, and ~525 historical appointments
 * from two CSV files into the live Supabase database.
 *
 * Usage:
 *   npx tsx scripts/import-csv-data.ts           # Dry run (default)
 *   npx tsx scripts/import-csv-data.ts --execute  # Actually write to DB
 */
...
const CUSTOMER_CSV = join(__dirname, '..', 'docs/import-files/puppyday-customer-pet-import.csv');
const APPT_CSV = join(__dirname, '..', 'docs/import-files/puppyday-appointments-import.csv');
...
async function main() {
  console.log('============================================');
  console.log('  CSV Data Import for The Puppy Day');
  console.log('============================================');
...
async function processCustomers(rows: Record<string, string>[]) {
  ...
  // Get existing customers by phone
...
async function processPets(
  ...
  // In execute mode, also check for existing pets to avoid duplicates
...
async function processAppointments(
  ...
  console.log(
    `  Appointment processing complete: ${result.inserted} to insert, ${fleaCount} with FLEA add-on`,
  );
...
function printSummary(...results: ImportResult[]) {
  ...
  if (!EXECUTE) {
    ...
    console.log('  npx tsx scripts/import-csv-data.ts --execute');
  }
