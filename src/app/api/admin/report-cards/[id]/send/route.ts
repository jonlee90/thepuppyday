...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    try {
      // Fetch appointment and report card images in parallel
      const [
      ...
      after(async () => {
        try {
          ...
          if (!result.success) {
            console.error('[Report Card Send] Notification dispatch failed:', result.errors);
          }
        } catch (error) {
      ...
      console.error('[Report Card Send] Error preparing report card send:', error);
      return NextResponse.json(
